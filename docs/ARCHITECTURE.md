...
| **Trade View** | V2 single-trade detail page. Navigated to programmatically via `navigateToTrade(id)` — no sidebar link. Fetches the trade fresh on every navigation via `trade:get`. Header shows a Back arrow (`goBack()` — returns to the previous page), the trade title, and Edit/Delete actions. Body: `TradeDetailContent` (3-column field grid + notes + 3-state screenshot block), `StrategyRulesSummary` (collapsible read-only rules panel, collapsed by default; shows "No rules defined" when none exist), and `AiReviewSection` (checks API key on mount; shows "Request AI Review" / "Re-request Review" button; spinner during call; renders persisted review prose with `whitespace-pre-wrap`; error + Retry on failure; seeded from `trade.review` / `trade.reviewCreatedAt` so persisted reviews appear immediately). Inline edit mode swaps the detail view for a pre-filled `TradeEntryForm` (fields only — screenshot changes are not supported in edit); Save calls `trade:update`, Cancel returns to the read-only view. Delete opens a confirmation dialog; on confirm calls `trade:delete` and navigates back. Invalid or missing trade ID renders an error state with a "Back to Dashboard" link. |
...
| Store | State | Purpose |
|---|---|---|
| `navigation-store.ts` | `activePage: Page`, `previousPage: Page`, `selectedTradeId: number \| null` | Tracks the active page; defaults to `'dashboard'`. `setPage(page)` records `activePage` into `previousPage` before switching. `navigateToTrade(id)` records `activePage` into `previousPage`, sets `activePage: 'trade-view'`, and sets `selectedTradeId` — all atomically. `goBack()` sets `activePage` to `previousPage` (single-level back; no full history stack). `Page` type: `'dashboard' \| 'trade-logger' \| 'trade-view' \| 'strategy-rules' \| 'knowledge-base' \| 'import-review' \| 'settings'` |
